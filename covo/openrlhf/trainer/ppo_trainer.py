...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.replay_buffer = NaiveReplayBuffer(
            micro_train_batch_size, strategy, buffer_limit, buffer_cpu_offload, packing_samples
        )
    ...
    def evaluate(self, global_step):
        ...
        for rand_prompts, labels, levels in self.eval_dataloader:
            resps = self.experience_maker._generate_resp(rand_prompts, **self.generate_kwargs)
            for pro, label, level, resp in zip(rand_prompts, labels, levels.tolist(), resps):
                wrap_label = label

                if f"level_{level}" not in level2acc:
                    raise Exception("Level not found")
                level2cnt[f"level_{level}"] += 1
                level2cnt["all"] += 1
                if check(wrap_label, resp):
                    level2acc[f"level_{level}"] += 1
                    level2acc["all"] += 1
        
        level2acc = self.strategy.all_reduce(level2acc)
        level2cnt = self.strategy.all_reduce(level2cnt)
        for level in level2acc:
            level2acc[level] /= level2cnt[level]
        # all是加权平均
        # level2acc['all'] = sum([level2acc[level] * level2cnt[level] for level in level2acc]) / sum(level2cnt.values())
        print(level2acc)
        if self.strategy.args.vllm_enable_sleep:
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        ...
        if global_step == 1 or global_step % args.eval_steps == 0:
            level2acc = self.evaluate(global_step)
            for level in level2acc:
                logs_dict[f'math_eval_{level}'] = level2acc[level]
